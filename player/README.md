...
6. 断网/断电应急：连不上服务器时会自动在电视上显示「本地模式」（酒店名 + 本地时钟），也可以点「🏠 本地模式」手动切换；服务器恢复后自动切回线上。

## 常见问题

- **画面没出现在电视上**：确认 HDMI 电视已连接、系统里设为「扩展」显示、方向设为竖屏。播放器会自动在「非主屏」上全屏。
- **断网/断电时电视显示什么**：优先显示缓存的最新房价牌；若完全没有缓存，显示本地应急时钟（酒店名 + 时间），每 30 秒自动尝试重连，恢复后自动切回。电视上不会出现任何报错文字。
